...
class AccountsController:
    ...
    async def get_account(self, by: Literal['id', 'number'], value: int | str) -> Record | None:
        ...
        if by not in ['id', 'number']:
            raise AttributeError(
                'the `by` argument value must be `id` or `number`.'
            )
        if not isinstance(value, int | str):
            raise TypeError(
                'the `value` argument must be int or str.'
            )
    ...
    async def get_account_type(self, id: int) -> Record | None:
        """fetch one account  type from database number or id

        Args:
            id (int): the account type id

        Raises:
            TypeError: if id is not instance of int
            AccountDatabaseException: if some sqlalchemy error occur
        ...
        if not isinstance(id, int):
            raise TypeError(
                '`id` must be instance of int'
            )

        try:
            stmt = select(self._account_type_model).where(self._account_type_model.id == id)
    ...
    def __check_fields(self, fields: Sequence[str], model: type[Account] | type[AccountType]):
        for field in fields:
